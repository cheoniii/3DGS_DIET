...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, dbscan_iteration, mode):
    # [함수 정의 및 초기 설정] # 
    ...
    for iteration in range(first_iter, opt.iterations + 1):   # 훈련 반복을 실행  opt.iterations + 1
        ...
        if iteration in dbscan_iteration:
          ...
          # cph.utility.Vector3fVector는 cluster_dbscan을 수행할 수 없음
          # 즉 pcl.points, pcl.colors를 직접적으로 cluster_dbscan을 할 수 없기 때문에 
          # xyz, color값만을 저장한 pcl(PointCloud)를 생성
          ...
          pcl1,pcl2, pcl3, pcl4 = dbscan.set_parameter(gaussians,mode)

          # points = pcl1.point.positions.numpy()
          # colors = pcl2.point.colors.numpy()

          for _ in tqdm(range(1), desc='DBSCAN....'):
            if mode == 1:  # 위치, 색상, 투명도 클러스터링
              labels = dbscan.dbscan(pcl1,0.1,5)  # dbscan(eps, min_points)
              points, xyz, f_dc = dbscan.cluster_mean(pcl1, pcl1,labels)
              opacities = dbscan.opacities_cluster(pcl1,labels,mode)
            elif mode == 2: # 색상, 투명도 클러스터링
              labels = dbscan.dbscan(pcl2,15,5)
              points, xyz, f_dc = dbscan.cluster_mean(pcl3, pcl2,labels)
              opacities = dbscan.opacities_cluster(pcl2,labels,mode)
            elif mode == 3: # 위치, 투명도 클러스터링
              labels = dbscan.dbscan(pcl3,0.1,5)
              points, xyz, f_dc = dbscan.cluster_mean(pcl3, pcl1,labels)
              opacities = dbscan.opacities_cluster(pcl2,labels,mode)
            elif mode ==4: # 위치, 색상 클러스터링
              labels = dbscan.dbscan(pcl4,0.1,5)
              points, xyz, f_dc = dbscan.cluster_mean(pcl4, pcl4,labels)
              opacities = dbscan.opacities_cluster(pcl3,labels,mode)

          scales,rots = dbscan.scale_rotaion(points,labels)

          gaussians.set_dbscan_parameter(xyz,f_dc,scales,rots,opacities)
          
          del pcl1,pcl2, pcl3, opacities, labels, points, xyz, f_dc, scales, rots
        ...
        current_memory_usage = torch.cuda.memory_allocated() / 1024 / 1024  # MB 단위로 변환
        max_memory_usage = torch.cuda.max_memory_allocated() / 1024 / 1024  # MB 단위로 변환
        ...
        with torch.no_grad(): # 모델의 평가가 아닌 부수적인 작업을 수행하는 데 사용
            ...
            if iteration < opt.densify_until_iter:               # 25000, opt.densify_until_iter
    ...
    wandb.log({"avg_memory_usage": total_memory_usage/opt.iterations, "iteration": iteration})
    ##################################################################################################
...
if __name__ == "__main__":
    ...
    ##################################################################################################
    parser.add_argument("--test_iterations", nargs="+", type=int, default=[7000, 10_000, 15_000, 30_000])
    parser.add_argument("--save_iterations", nargs="+", type=int, default=[7_000, 15_000, 30_000])
    ##################################################################################################
    parser.add_argument("--dbscan_iteration", type=int, default=[10000,20000])
    ...
    training(lp.extract(args), op.extract(args), pp.extract(args), args.test_iterations, args.save_iterations, args.checkpoint_iterations, args.start_checkpoint, args.debug_from, args.dbscan_iteration, args.mode)
